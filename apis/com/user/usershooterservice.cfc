<cfcomponent displayname="usershooterservice">
	...
	<cffunction name="getGameCheckout" access="public" returntype="query" output="false" hint="I get the shooter game statuses for checkout.">
		<cfargument name="vsid" type="numeric" required="yes" default="#session.vsid#">
		<cfset gamecheckout = "" />
			<cfquery name="gamecheckout">
				select count(distinct(gs.gameid)) as totalgamestatus
				  from gamestatus gs, games g
				 where gs.gameid = g.gameid
			       and g.vsid = <cfqueryparam value="#arguments.vsid#" cfsqltype="cf_sql_integer" />		  
			</cfquery>
		<cfreturn gamecheckout>
	</cffunction>
			...
			<cfquery name="gamescomplete">
				select sa.shooterassignmentid, sa.vsid, sa.shooterassignstatus, sa.shooterassignlastupdated,
				 ...
				 where sa.shooterid = s.shooterid
				   ...
				   and v.fieldid = f.fieldid
				   and s.userid = <cfqueryparam value="#arguments.userid#" cfsqltype="cf_sql_integer" />
				   and sa.shooterassignstatus = <cfqueryparam value="Completed" cfsqltype="cf_sql_varchar" />
				   and sa.shooteracceptedassignment = <cfqueryparam value="1" cfsqltype="cf_sql_bit" />
			  order by sa.shooterassignmentid desc
			</cfquery>
